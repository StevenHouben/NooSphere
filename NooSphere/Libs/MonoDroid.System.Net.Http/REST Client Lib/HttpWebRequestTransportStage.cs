...
namespace Microsoft.Http
{
    ...
    public class HttpWebRequestTransportStage : HttpAsyncStage
    {
        ...
        {
          string[] keys = webResponseHeaders.AllKeys;
          for (int i = 0; i < keys.Length; ++i)
          {
            var k = keys[i];
            var v = webResponseHeaders[k];
            ...
            foreach (string key in source.Keys)
            {
                ...
                switch (key.ToUpper(CultureInfo.InvariantCulture))
                {
                    ...
                    case "CONTENT-TYPE":
                        ...
                        break;
                    case "CONNECTION":
                        throw new NotSupportedException("Not supported in the SL client.");
                    case "CONTENT-LENGTH":
                        //NOTE Removed in SL client
//                        request.Headers[key] = source.ContentLength.Value.ToString();
                        break;
                    case "EXPECT":
                        throw new NotSupportedException("Not supported in the SL client.");
        ...
        static void CopySettingsToHttpWebRequest(HttpWebRequestTransportSettings settings, HttpWebRequest request)
        {
          ...
          //NOTE DISABLED FUNCTIONALITY IN SL CLIENT
        ...
        class HttpTransportAsyncResult : AsyncResult<HttpResponseMessage>, IHttpCancel
        {
                ...
                : base(callback, state)
            {
                ...
                if (!HttpContent.IsNullOrEmpty(request.Content))
                {
                    ...
                    {
                      //NOTE changed IN SL CLIENT
                      var timeout = settings.ConnectionTimeout ?? TimeSpan.FromSeconds(15);
                      stream = this.webRequest.GetRequestStream((int)timeout.TotalMilliseconds);
                    }
            ...
            void RegisterAbortTimeout(IAsyncResult result, TimeoutReason reason)
            {
                ...
                //NOTE changed to fixed value in SL client
                TimeoutHelper.Register(result.AsyncWaitHandle, MaybeAbortCallback, this, TimeSpan.FromMilliseconds(10000));
            }
...
//                    } 
                    ...
                    if (we.Status == WebExceptionStatus.RequestCanceled || self.cancelled)
                    {
                        self.cancelled = true;
...
//                }
                ...
                {
                  throw new NotSupportedException("Currently not implemented in SL client");
