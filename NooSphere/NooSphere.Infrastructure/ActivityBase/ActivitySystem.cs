...
namespace NooSphere.Infrastructure.ActivityBase
{
	...
	private void HandleUnfoundType<T>()
        ...
        public FileResource AddFileResourceToActivity(Activity activity, byte[] data, string path, string type)
        {
        ...
        public void AddFileResourceToActivity( Activity activity,Stream stream,string path,string type)
        {
           var resource = new FileResource()
            {
                FileType =  type,
